        ...
        updateTabBar() {
            ...
            if (pages.length > 0) {
                ...
                if (route) {
                    ...
                    const adminIndex = this.data.adminList.findIndex((item) => item.pagePath === fullPath);
                    const driverIndex = this.data.driverList.findIndex((item) => item.pagePath === fullPath);
                    if (adminIndex >= 0) {
                        isAdmin = true;
                    }
            ...
            if (pages.length > 0) {
                ...
                if (route) {
                    ...
                    selected = list.findIndex((item) => item.pagePath === fullPath);
                    if (selected < 0)
                        selected = 0;
                }
            }
